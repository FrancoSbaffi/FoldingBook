...
/// Registers a global hotkey (Control + Command + L) to toggle the effect.
final class GlobalShortcut {
    private var hotKey: EventHotKeyRef?
    private var handler: EventHandlerRef?
    private let action: () -> Void
    private static let signature: OSType = 0x46424F4B // 'FBOK'
    ...
    private static func error(_ status: OSStatus) -> NSError {
        NSError(domain: "FoldingBook.Shortcut", code: Int(status), userInfo: [NSLocalizedDescriptionKey: "Could not register Control–Command–L (\(status)). Another app may already use it."])
    }
