...
final class PlaneRenderer: NSObject, MTKViewDelegate {
    ...
    private let logger = Logger(subsystem: "com.foldingbook.app", category: "Renderer")
    ...
    private func bindTextures(_ encoder: MTLRenderCommandEncoder, source: MTLTexture) {
        ...
        for index in 0..<4 {
            encoder.setFragmentTexture(blurLevels.indices.contains(index) ? blurLevels[index] : source, index: index + 1)
        }
    }
    ...
    func draw(in view: MTKView) {
        attemptedDraws += 1
        tick?()
        // NEVER draw artwork in the live overlay: only real desktop frames!
        guard let desktop = desktopTexture.flatMap(CVMetalTextureGetTexture) else {
            return
        }
        ...
        if blur && abs(delta) > 0.003 { prepareBlur(command, source: desktop) }
        ...
        encoder.setRenderPipelineState(pipeline)
        bindTextures(encoder, source: desktop)
        encoder.setFragmentBytes(&params, length: MemoryLayout.size(ofValue: params), index: 0)
        encoder.drawPrimitives(type: .triangle, vertexStart: 0, vertexCount: 3)
        encoder.endEncoding()
    ...
    static func artwork() -> CGImage {
        ...
        text("FOLDING DISPLAY PROJECTION", 110, 867, 20, mint, .medium)
        text("FoldingBook", 100, 710, 116, .white, .semibold)
        text("Fold the lid. Watch the content maintain its ground.", 110, 650, 28, .white.withAlphaComponent(0.7))
        let cards: [(CGFloat, String, String, NSColor)] = [
            (110, "01", "Fixed Plane", mint),
            (580, "02", "Moving Surface", NSColor(red: 0.72, green: 0.79, blue: 1, alpha: 1)),
            (1050, "03", "Progressive Blur", NSColor(red: 1, green: 0.75, blue: 0.55, alpha: 1))
        ]
    ...
    fragment float4 fragmentMain(VertexOut in [[stage_in]],
        ...
        float3 color;
        if (radius < 2.0) {
            color = mix(art.sample(s, uv).rgb, b1.sample(s, uv).rgb, radius / 2.0);
        } else if (radius < 6.0) {
            color = mix(b1.sample(s, uv).rgb, b2.sample(s, uv).rgb, (radius - 2.0) / 4.0);
        } else if (radius < 16.0) {
            color = mix(b2.sample(s, uv).rgb, b3.sample(s, uv).rgb, (radius - 6.0) / 10.0);
        } else {
            color = mix(b3.sample(s, uv).rgb, b4.sample(s, uv).rgb, clamp((radius - 16.0) / 24.0, 0.0, 1.0));
        }
