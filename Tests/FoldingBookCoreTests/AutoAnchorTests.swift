...
@main struct AutoAnchorTests {
    static func main() {
        ...
        print("PASS: all debounce, jitter, interruption and manual-anchor checks passed")
    }
    ...
    func testSlowCumulativeMovementRestartsDebounce() {
        var state = AutoAnchor(angle: 110, now: 0)
        for i in 1...10 {
            state.update(angle: 110 - Double(i), now: Double(i) * 0.3, enabled: true)
        }
        XCTAssertEqual(state.reference, 110)
    }
    ...
    func testSmallSensorJitterDoesNotPreventSettling() {
        ...
        state.update(angle: 80, now: 0.1, enabled: true)
        for i in 2...20 {
            state.update(angle: 80 + (i.isMultiple(of: 2) ? 0.3 : -0.3), now: Double(i) * 0.1, enabled: true)
        }
